...
export default class product extends React.Component{
	getProChildren = (data) => {
	 	...
	 	const FileLink = data.contentLink.split(/\n/).filter(item => item);
			 		...
			 		<Col lg={6} xs={12} key={ii} className="proBox">
			 			<Card style = {{width:240}} bodyStyle ={{padding:10}} className="contentCard">
			 				...
			 				<div className = "custom-card">
			 					<h3>{cTitle[ii]}</h3>
			 					<p><Link to={`${this.props.match.url}/{FileLink[ii]}`}>查看文档 </Link></p>
			 				</div>
			 			</Card>
			 		</Col>
		 		);
		    ...
		    <Col lg={3}>
		      ...
		      >
			    ...
			    <Col lg={19} offset={2} className="contentBox">
			      ...
			      <Route path={`${this.props.match.url}/:topicId`} component={productFile}/>
		      </div>
    );
	}
}