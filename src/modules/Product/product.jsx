...
export default class product extends React.Component{
	 		...
	 		.map((item,ii) => {
	 			...
	 			return (
	 				<Col lg={8} xs={8} key={ii} className="proBox">
			 			<Card style = {{width:'80%'}} bodyStyle ={{padding:20}} className="contentCard" onClick={ () => this.showModal(cTitle[ii],cIntroduce[ii])}>
	...
	render(){
		...
		const MilChildren = cTitle.map((data,i) => (<Col lg={8} xs={8} key={i} className="proBox"><Button style={{width:'80%',height:'45px'}}>{cTitle[i]}</Button></Col>));
